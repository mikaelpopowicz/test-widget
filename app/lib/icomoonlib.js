...
function initializeFont(fontname) {
	...
	try {
		...
		for (var i = 0; i < fontmap.length; i++) {
			...
			//Ti.API.log("Found: " + fontname + " / " + font + " / " + code);
			fontMaps[fontname][font] = String.fromCharCode(code);
		}
	} catch (fontParseError) {
		Ti.API.log("*** There was a font parsing error.  " + "Did you copy your font's selection.json file " + "into the assets folder of your application and name it " + fontname + ".json?");
		Ti.API.log("*** fontParseError: " + fontParseError);
	}
}
...
var getIconAsLabel = function(fontname, iconname, size, options) {
	...
	//alert(fontMaps[fontname][iconname]);
	return label;
};
...
var getIcon = function(fontname, iconname, size, options) {
	...
	//Ti.API.log("icon filename:" + filename);
	var path = null;
	if (Ti.Android) {
		path = Ti.Filesystem.getFile(Ti.Filesystem.externalStorageDirectory, filename);
	}else{
		path = Ti.Filesystem.getFile(Ti.Filesystem.applicationCacheDirectory, filename);
	}
	
	if (path.exists()) {
		//Ti.API.log("*** returning cached version");
		//alert(path.nativePath);
		return path.nativePath;
		
	} else {
		//Ti.API.log("*** creating new file: " + path.nativePath);
		var blob = getIconAsBlob(fontname, iconname, size, options);
		//Ti.API.log(blob.apiName);
		if (Ti.Android) {
		...
		//Ti.API.log("hello path : "+ path.nativePath);
		return path.nativePath;
	}
};
